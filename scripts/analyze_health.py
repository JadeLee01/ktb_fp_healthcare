...
class Config:
    ...
    # -------------------------
    # Stance constraints (Base)
    # -------------------------
    MIN_HEIGHT_SCALE_PX = 5.0 # Minimum Scale Check
    Q_VEL_TH_BASE = 35   # Base Velocity Threshold (Strict)
    Y_EPS_RATIO_BASE = 0.10 # Base Height Tolerance (Strict)
    
    # -------------------------
    # Stance constraints (Relaxed Retry)
    # -------------------------
    Q_VEL_TH_RETRY = 65  # Relaxed Velocity Threshold
    Y_EPS_RATIO_RETRY = 0.15 # Relaxed Height Tolerance
    ...
    LOW_STANCE_THRESHOLD = 0.10 # If sum(L+R) < 0.10, trigger retry/unknown
...
class DogHealthAnalyzer:
    ...
    def _error(self, aid, did, code):
        """Standard Error Response: Only Error Code"""
